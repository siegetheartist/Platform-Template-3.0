...
function scr_player_get_input() {
    ...
    for (var i = 0; i < _gp_count; i++) {
        if (gamepad_is_connected(i)) {
            ...
            var _v_axis = gamepad_axis_value(i, gp_axislv);
            var _deadzone = 0.2;
            ...
            _gp_down_held += gamepad_button_check(i, gp_padd) || (_v_axis > _deadzone);
    ...
    // --- Combine keyboard + gamepad inputs, clamp to [0,1] to avoid double-counting ---
    _input_data.left_held = clamp(_key_left_kb_held + _gp_left_held, 0, 1);
    _input_data.right_held = clamp(_key_right_kb_held + _gp_right_held, 0, 1);
    _input_data.down_held = clamp(_key_down_kb_held + _gp_down_held, 0, 1);
